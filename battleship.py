...
def get_ship_placement(board, size):
    while True:
        try:
            position = input(f"Enter the row (A-{chr(65+len(board)-1)}) and column (1-{len(board)}) for a ship of size {size}: ")
...
def get_shot_position(board):
    while True:
        try:
            position = input(f"Enter the row (A-{chr(65+len(board)-1)}) and column (1-{len(board)}) to shoot: ")
            row = ord(position[0]) - 65
            col = int(position[1:]) - 1
...
def ai_play_game(board_size, ship_sizes):
    ...
    for size in ship_sizes:
        ...
        col= random.randint(0,board_size)
        directio
        place_ship(board, row, col, size, direction)
    display_board(board)
    if input("Press enter to continue or q to quit: ") == "q":
